    ...
    for (entity, pos) in (&entities, &pos_storage).join() {
        ...
        let hero_ext = if matches!(kind, EntityKind::Hero) {
            ...
            let buffs: Vec<BuffSnapshot> = buff_store
                .iter_for(entity)
                .map(|(id, entry)| {
                    let remaining_secs = buff_remaining_secs_for_snapshot(entry.remaining);
                    BuffSnapshot {
            ...
            // Phase 4.5：每個 slot 的 ability id + level（W/E/R/T = 0..3）。
            // `Hero.abilities` 是 `Vec<String>`（通常長度為 4，但仍防禦較短情境）；
            // `ability_levels` 是以 ability id 為 key 的 HashMap。缺值 → 0 / None。
